...
if (interactive && !CanWrite(dataDir) && !IsAdmin())
{
    ...
    {
        Console.WriteLine("Preciso de permissão para gravar na pasta do jogo.");
        Console.WriteLine($"Tente rodar o programa com 'sudo ./Instalar_Traducao_Linux --data \"{dataDir}\"'.\n");
        return 1;
    }
...
string bundleDir = Path.GetDirectoryName(bundlePath) ?? Path.GetTempPath();
string tmpUncomp = Path.Combine(bundleDir, $"fsptbr_uncomp_{Guid.NewGuid():N}.tmp");
string lz4Out = testOut ?? Path.Combine(bundleDir, $"fsptbr_patched_{Guid.NewGuid():N}.unity3d");
