...
foreach (var t in terms.Children)
{
    ...
    if (!trans.TryGetValue(key, out var ptbr)) { 
        notFound++; 
    ...
    {
        slots.Children[i].AsString = ptbr;
    }
    applied++;
}
Console.WriteLine($"Aplicadas: {applied} (em todos os idiomas) | termos sem tradução no CSV: {notFound}");
